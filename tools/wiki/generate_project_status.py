#!/usr/bin/env python3
"""Generate the ``wiki/Project-Status.md`` index page.

Indexes the in-repo status documents now stored as standalone wiki pages,
and optionally overlays live data from a GitHub Projects (v2) board. The
board overlay is **best-effort**: when ``PROJECTS_READ_TOKEN`` is
...
# Status pages to list, in display order. Missing files are skipped.
STATUS_SOURCES = [
    "wiki/Quick-Status.md",
    "wiki/Project-Status-Report.md",
    "wiki/Design-vs-Implementation.md",
]
...
def status_index(repo_root: Path) -> str:
    rows: list[str] = []
    for rel in STATUS_SOURCES:
        p = repo_root / rel
        if not p.is_file():
            continue
        rows.append(f"- [{_page_title(p)}]({p.stem})")
    if not rows:
        return "_No status documents found under `wiki/`._\n"
    return (
...
def build_page(repo_root: Path) -> str:
    ...
    return (
        ...
        "# Project Status\n\n"
        f"_Generated {now} for `{repo}`. Indexes status pages in `wiki/` plus an optional live project board._\n\n"
        "## Live Project Board\n\n"
        f"{project_board_section()}\n"
        "## In-Repo Status\n\n"
        f"{status_index(repo_root)}\n"
    ).rstrip() + "\n"
