...
Aggregates the in-repo status documents now stored as wiki pages into one
...
# Status pages to aggregate, in display order. Missing files are skipped.
STATUS_SOURCES = [
...
def _strip_wiki_metadata(text: str) -> str:
    """Drop leading wiki topic/title comments before embedding a source page."""
...
def aggregate_status(repo_root: Path) -> str:
    ...
    for rel in STATUS_SOURCES:
        ...
        if not p.is_file():
            continue
        body = _demote_headings(_strip_wiki_metadata(p.read_text(encoding="utf-8")), by=1)
...
def build_page(repo_root: Path) -> str:
    ...
    return (
        ...
        "# Project Status\n\n"
        f"_Generated {now} for `{repo}`. Aggregates status pages in `wiki/` plus an optional live project board._\n\n"
        "## Live Project Board\n\n"
