#!/usr/bin/env python3
"""Generate wiki/_Sidebar.md from the SolaceCore wiki folder structure."""
...
def build_sidebar(wiki_dir: Path) -> str:
    ...
    for topic in ordered(set(groups), SECTION_ORDER):
        ...
        for subtopic in ordered(subtopic_names, SUBTOPIC_ORDER.get(topic, [])):
            lines.append("  <details>")
            lines.append(f"  <summary>{subtopic}</summary>")
            lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate SolaceCore wiki sidebar")
    ...
    content = build_sidebar(wiki_dir)
    (wiki_dir / "_Sidebar.md").write_text(content, encoding="utf-8")
    print(f"Generated _Sidebar.md with {content.count('<details>')} collapsible groups")
    return 0
