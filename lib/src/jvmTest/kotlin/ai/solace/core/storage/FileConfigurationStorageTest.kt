...
class FileConfigurationStorageTest {
    ...
    private lateinit var configStorage: FileConfigurationStorage

    @BeforeTest
    fun setup() {
        // Create a temporary directory for testing
        tempDir = Files.createTempDirectory("file-config-storage-test").toString()
        
        // Create a FileConfigurationStorage instance
        configStorage = FileConfigurationStorage(tempDir)
    }
    ...
    fun testPersistence() = runBlocking {
        withTimeout(5.seconds) {
            ...
            // Create a new FileConfigurationStorage instance with the same base directory
            val newConfigStorage = FileConfigurationStorage(tempDir)
            
            // Verify that the config value is still available
            assertEquals(8080, newConfigStorage.getConfigValue("server", "port"))
        }
    }
}