...
/**
 * In-memory implementation of the ConfigurationStorage interface.
 *
 * This implementation stores configuration data in memory using a mutable map.
...
class InMemoryConfigurationStorage : InMemoryStorage<String, Map<String, Any>>(), ConfigurationStorage {
    ...
    override suspend fun setConfigValue(key: String, path: String, value: Any): Boolean {
        // Validate the path before acquiring the mutex
        if (!isValidPath(path)) {
            return false
        }

        // Retrieve the current config outside the mutex lock
        ...
        // Now set the value and store with mutex lock
        return try {
            ...
            mutex.withLock {
                storage[key] = Pair(config, metadata)
                true
            }
        } catch (e: Exception) {
