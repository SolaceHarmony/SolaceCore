...
sealed class Port<T : Any>(val name: String, val type: KClass<T>) {
    class Input<T : Any>(name: String, type: KClass<T>) : Port<T>(name, type) {
        ...
        suspend fun receive(): Result<T?> {
            return runCatching { channel?.receive() }
        }
    }
    ...
    class Tool<T : Any>(name: String, type: KClass<T>) : Port<T>(name, type) {
        ...
        suspend fun invoke(param: T): Result<Any> {
            return runCatching {
                implementation?.invoke(param) ?: throw IllegalStateException("Tool $name has no implementation")
            }
        }
    }
}