...
Model/provider integration and MCP tool surface the actors can call.

**Where it fits**
The companion's connection to models and external tools.

**Primary sources (in-repo)**
- `Providers-and-MCP-Tools`
- `Providers-and-MCP-Tools`
...
- **The [pipeline DSL](Pipeline-DSL)** — the API style determines
  ...
  parsing.
- **The [MCP & Tool Format layer](Providers-and-MCP-Tools)** — the tool
...
- [mcp-tools](Providers-and-MCP-Tools) — tool support sub-spec governs the
...
(Supervisor-AI). Steps 2 and 7 above pass through the
...
- [supervisor](Supervisor-AI) — gates MCP requests through the
...
- [providers](Providers-and-MCP-Tools) — provider abstraction reads which
  ToolFormat each provider's models speak.
- [pipeline](Pipeline-DSL) — the pipeline DSL composes tool
