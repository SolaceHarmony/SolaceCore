...
SolaceCore utilizes Qodana, JetBrains' static analysis engine, for comprehensive code quality checks. The configuration is managed via the `qodana.yaml` file in the project root.

Key configuration aspects from `qodana.yaml` include:
...
The `qodana.yaml` file also provides commented-out sections for more advanced configurations, such as:
